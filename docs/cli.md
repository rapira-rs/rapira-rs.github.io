...
So a flag always wins over the same value in `rapira.toml`, and `rapira.toml` always wins over the default. That ordering is what makes `--config` useful in practice: keep the stable configuration in the file, then override one value on the command line for a single run — a different port while you test, more workers on a bigger box — without editing anything.

Anything you don't set at all falls through to the defaults in the table below. The full list of what a config file can hold lives on [Configuration](/docs/configuration).
...
\* Required unless the config file sets `pool.entrypoint`. With neither, `serve` refuses to start and says so.

**`--listen`** takes three shapes. `127.0.0.1:8000` (the default) binds one interface — loopback only, so nothing outside the machine can reach it. `:8080` is shorthand for `0.0.0.0:8080` — every IPv4 interface, which is what you want in a container; for IPv6 write `[::]:8080`. `unix:/run/rapira.sock` binds a Unix socket instead, for a reverse proxy on the same host. IPv6 literals go in brackets: `[::1]:8000`. A bare port is *not* an address and is rejected — `--listen 8080` is an error, write `--listen :8080`. The host part has to be an IP literal — hostnames are never resolved, so `--listen localhost:8000` is an error; write `--listen 127.0.0.1:8000`.
...
The script can be given twice — as the positional `SCRIPT` argument or as `pool.entrypoint` in the config file — and if both are present, the command line wins. Either way Rapira turns it into an absolute path before the server forks anything, because a daemon's working directory is not the directory you deployed into.
...
A handful of invocations that cover most of what you'll actually type:
...
::: question Why is `--listen 8080` rejected?
Because a bare port doesn't say which interfaces to bind, and Rapira would have to guess between loopback and everything. Spell out what you mean: `--listen :8080` for every IPv4 interface, `--listen 127.0.0.1:8080` for loopback only.
:::

::: question Do I need a config file at all?
No. Flags alone are enough to run a server, and everything you don't set has a default. Reach for `--config` when you need what flags don't expose — pool scaling, logging, request limits — all of which are described on [Configuration](/docs/configuration).
:::

::: question I passed `--config` and a `SCRIPT`. Which script runs?
The one on the command line. Flags outrank the file, so the positional `SCRIPT` overrides `pool.entrypoint` while every other setting in the file still applies — handy for pointing an existing config at a different entry script for one run.
:::
