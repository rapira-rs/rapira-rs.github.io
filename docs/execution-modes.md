...
description: Rapira's four-rung ladder — Classic, SAPI Worker, PSR Worker and Async — what each rung means, and what decides how high your application can climb.
---

# Execution modes

Every PHP server has to answer one question: how much of your application survives between two requests? Under php-fpm the answer is "nothing" — the framework boots from zero every time, and for a modern application that boot is often the most expensive part of the request. Rapira does not force a single answer on you. It offers a ladder of four execution modes, and an application climbs as high as it can.
...
The names describe the rung itself — whether the worker stays alive, and what contract it speaks — rather than the product that made the shape familiar. The higher the rung, the more of your process is warm when a request arrives, and the more the code has to be able to cope with that.
...
This is the compatibility rung. An existing application runs as it is — Rapira takes php-fpm's place with no changes to your code, and the win is in the plumbing rather than in your app: PHP is embedded in the server process, so there is no FastCGI hop between the HTTP front and the interpreter.

See [Classic mode](/docs/classic) for how to run it.

## SAPI Worker <Badge type="tip" text="shipped" />

The same shape as Classic — you still read the superglobals, still `echo` your response — except the worker does not die at the end of a request. A resident script boots everything once, then loops: the server refills `$_GET`, `$_POST`, `$_SERVER`, `$_COOKIE` and friends for each new request, runs your handler, and hands you the next one. Autoloader, DI container, configuration, database connections — anything created outside the loop stays warm.

That is the whole point of the rung: the boot cost is paid once per worker instead of once per request. What you give up is amnesia. Whatever your application leaves lying around in static properties, singletons or global state is still there on the next request, which is exactly what makes this rung a property of your code rather than a switch.
...
On this rung the PHP side stops being called and starts asking: the worker pulls a request from Rapira through an API call and decides what to do with it. It can fill the superglobals for compatibility, or skip them entirely and work with a PSR-7 message it passes straight to a framework's HTTP kernel. One request at a time, same as the rung below.

The gain is that the request stops being ambient global state and becomes a value you can pass around, wrap, or hand to a middleware stack — which is how modern PHP frameworks want to receive it in the first place.
...
This is the top of the ladder and the least forgiving rung — concurrency inside one interpreter means every library in the request path has to behave when it is suspended halfway through.

::: info
This rung is a concept too: there is nothing to install and nothing to configure. Read the section above as a description of where Rapira is heading, rather than of something you can run today.
:::
...
Global state that cannot survive a second request keeps you on Classic. A library that is not fiber-safe keeps you below Async. A framework with a runtime integration hands you the SAPI Worker rung almost for free — see [Frameworks](/docs/frameworks/) for the ones with a documented path. In every case that is a property of the code, not a restriction Rapira imposes: the whole ladder is there, and the application decides how high it climbs.

Climbing is a one-way change only in the sense that it costs work on the PHP side — a worker rung needs a resident entry script that the Classic rung does not. Falling back is always safe: turn Classic back on — a flag on the command line or a single key in the config file — point Rapira at your ordinary front controller, and you are on the Classic rung, with the same server, the same binary and the same [process model](/docs/process-model) underneath.
