...
Each worker runs one NTS PHP interpreter and an asynchronous HTTP server. The server uses hyper on a private tokio runtime with two threads.
Each worker calls `accept()` on its inherited socket. The operating system assigns each incoming connection to one worker.
...
::: info
The master retains the PHP module for its complete lifetime. Only the master shuts down the module.
A worker exits without shutting down this shared engine state.
:::
...
- After a failure, the replacement delay starts at 100 ms. It doubles after each consecutive failure and stops increasing near 25 seconds.
...
- After the pool serves a request, the master uses the normal replacement delay. A failed reload does not stop the existing workers.
...
- With `ondemand`, maintenance removes workers after the idle limit. An incoming connection causes worker creation.
- **Master monitoring.** Each worker reads from a pipe that the master keeps open.
- If the master exits, the pipe reaches EOF and each worker stops accepting work. Thus, a master failure does not leave unmanaged workers.
...
In Classic mode, the entry script executes in a new PHP request each time. New code takes effect without a reload.
However, `opcache.validate_timestamps = 0` requires a complete restart.
