...
**Peticiones que no terminan nunca.** Un worker atascado en una petición colgada se queda ahí indefinidamente y mientras tanto no atiende ninguna otra. `pool.request_terminate_timeout_secs` pone un límite de tiempo real a una sola petición y mata al worker que se lo salte. Las dos claves están en [Configuración](/es/docs/configuration), y lo que hace el proceso maestro cuando muere un worker, en [Modelo de procesos](/es/docs/process-model).
