...
- **Potrzebujesz innego zestawu rozszerzeń PHP.** Wydania niosą ze sobą PHP skompilowane z listy flag w pliku [`ci/php-configure-flags.txt`](https://github.com/rapira-rs/rapira/blob/main/ci/php-configure-flags.txt), celowo krótkiej: session, mbstring, OPcache, OpenSSL, curl, rodzina XML, PDO z SQLite. Jeśli twoja aplikacja chce `pdo_mysql`, `intl` albo `gd`, zbuduj Rapirę na PHP, które je ma.
...
- **Rusta z kanału stable.** Wersję przypina `rust-toolchain.toml` w repozytorium, więc [rustup](https://rustup.rs/) sam sięgnie po właściwą — niczego nie musisz przełączać.
...
::: tip
`make test` uruchamia zestawy testów i sam żongluje ścieżkami do bibliotek: znajduje bibliotekę embed w prefiksie z `php-config` (`lib`, `lib64`, `lib/phpXX`, nazwa zwykła albo wersjonowana) i sprowadza ją do zwykłej nazwy, której oczekuje linker. Dobry sposób, żeby upewnić się, że całość działa, zanim zaufasz swojej binarce.
:::
