...
| `classic` | wartość logiczna | `false` | `false` zostawia workera przy życiu między żądaniami (tryb SAPI Worker); `true` wykonuje skrypt wejściowy od zera przy każdym żądaniu, dokładnie tak jak php-fpm. Zobacz [tryby wykonania](/pl/docs/execution-modes). Flaga `--classic` potrafi tryb wyłącznie włączyć — `true` z pliku nie da się nadpisać z wiersza poleceń. |
