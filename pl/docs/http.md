...
::: warning
To nie ciekawostka, tylko sedno problemu. Jeśli zaufane proxy przed Rapirą ustawia `X-Forwarded-For`, to klient, który wyśle `X_Forwarded_For`, trafi pod ten sam klucz `$_SERVER` — a filtr nagłówków w proxy, wycinający pisownię z myślnikami, wersji z podkreśleniem w ogóle nie zauważy. Klient wpisuje wtedy wartość, którą twoja aplikacja bierze za pochodzącą od proxy.
:::

## Nazwy kolidujące ze zmienną CGI

Dlatego Rapira prześwietla nazwy pól żądania, zanim spojrzy na nie cokolwiek innego. Nazwa przechodzi, gdy każdy jej bajt mieści się w `[A-Za-z0-9-]`. Realnie kolidują dwa znaki, `_` i `.` — oba sprowadzają się do klucza `$_SERVER`, który należy do nazwy z myślnikiem. Reguła jest jednak listą znaków dozwolonych, a nie listą tych dwóch zakazanych: odrzuca też znak legalny, ale nietypowy, choćby `~`, i pozostanie poprawna, gdyby któreś z mapowań kiedyś się rozszerzyło. O tym, co dzieje się z odrzuconą nazwą, decyduje `http.unsafe_field_names`:
...
::: tip
`drop` loguje każde usunięcie na poziomie `warn`, ale domyślny poziom logowania to `error`, więc tych linii nie zobaczysz, dopóki go nie podniesiesz. Jeśli w `$_SERVER` w tajemniczy sposób brakuje nagłówka, podnieś poziom i zajrzyj najpierw do wpisów z targetem `http` — jak to zrobić, pokazują [Logi](/pl/docs/logging).
:::
...
Wartości pól docierają do PHP wszędzie jako surowe bajty. Ciasteczko w latin1 czy podpisany nagłówek zachowują każdy oktet wysłany przez klienta, bo konwersja na UTF-8 w dobrej wierze zepsułaby dokładnie te wartości, które zmienić się nie mogą.

## Treść żądania

Treść żądania trafia do pamięci, zanim ruszy PHP, a `http.max_body_size_mb` ogranicza, ile Rapira zgodzi się jej przetrzymać. Domyślnie jest to 8 MiB — tyle samo, ile wynosi domyślne `post_max_size` w samym PHP. Treść ponad limit dostaje `413`, a ponieważ reszta wciąż płynie do serwera, ta odpowiedź od razu zamyka połączenie, zamiast próbować użyć go ponownie.

Limit sprawdzany jest dwa razy, a to ważniejsze, niż brzmi:

- Najpierw wobec zadeklarowanego `Content-Length`, zanim wczytany zostanie choćby jeden bajt treści.
- Potem jeszcze raz w trakcie odbierania treści, kawałek po kawałku. Żądanie chunked nie deklaruje długości z góry, więc to drugie sprawdzenie jest jedyną rzeczą, która dzieli je od nieograniczonego zużycia pamięci.

W żądaniach HTTP/1.1 Rapira honoruje `Expect: 100-continue`: odsyła tymczasową odpowiedź `100 Continue`, a klient dopiero wtedy wysyła wstrzymaną treść. Cała wartość tkwi w kolejności — sprawdzenie `Content-Length` idzie *pierwsze*, więc klient zapowiadający zbyt dużą treść dostaje `413`, zanim cokolwiek wyśle. Oczekiwanie zgłoszone w żądaniu HTTP/1.0 jest ignorowane, tak jak wymaga tego [RFC 9110 §10.1.1](https://www.rfc-editor.org/rfc/rfc9110#section-10.1.1).
...
Jeśli PHP mimo wszystko wyśle nagłówek `Connection`, wycinane są także pola, które w nim wymieniono — dokładnie to znaczy wartość `Connection` — a dzieje się to, zanim Rapira wstawi swój własny `Content-Length`. Dzięki temu `Connection: content-length` nie zabierze treści jej własnej długości.

Cała reszta przechodzi tak, jak zapisało ją PHP, z powtórzeniami włącznie: `Set-Cookie`, `Vary` i `Link` mogą się prawidłowo pojawić kilka razy i wszystkie zostaną wysłane. Nagłówek, którego w ogóle nie da się przesłać, znika z wpisem w logu, zamiast wywracać całą odpowiedź — jedno felerne pole nie może cię kosztować treści.

## Wcześniejsze zakończenie odpowiedzi

Czasem to, co należy do klienta, kończy się długo przed samym żądaniem. Odpowiedź jest gotowa, ale zostaje jeszcze webhook do wystrzelenia, wpis do kolejki, cache do rozgrzania. Kazać przeglądarce na to czekać to czyste opóźnienie, z którego nikt nic nie ma.

`rapira_finish_request()` kończy odpowiedź w tym samym miejscu. Zbuforowane wyjście zostaje opróżnione, odpowiedź trafia do frontu i wychodzi do klienta, a twój handler biegnie dalej, gdy klient ma już całość u siebie. To ten sam kontrakt co `fastcgi_finish_request()`, więc kod pisany pod php-fpm zachowuje się dokładnie tak jak zawsze:
...
- **Wyjście po wywołaniu nie dociera do klienta.** Odpowiedź jest zamknięta, więc `echo` po tej linii trafia w próżnię — nie czeka w kolejce na późniejsze opróżnienie bufora, po prostu przepada. Wszystko, co klient ma zobaczyć, musisz wypisać wcześniej.
...
Tak — funkcja jest zarejestrowana dla całego procesu i działa na aktualnie obsługiwanym żądaniu, więc zachowuje się tak samo niezależnie od tego, czy skrypt jest rezydentny, czy wykonuje się od nowa przy każdym żądaniu. Co jeszcze zmienia się przy wspinaczce po drabinie, opisują [Tryby wykonania](/pl/docs/execution-modes).
:::
