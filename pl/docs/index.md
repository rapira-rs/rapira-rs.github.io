...
Cała różnica tkwi w tym, co stoi między serwerem a interpreterem: nic. Rapira osadza PHP we własnym procesie przez embed SAPI, ten sam interfejs, dzięki któremu program w C może hostować silnik PHP. Host wywołuje interpreter wprost: żadnego protokołu FastCGI, żadnego lokalnego gniazda ani potoku, żadnej serializacji do formatu sieciowego i z powrotem przy każdym żądaniu. Przychodzi żądanie: wypełniają się superglobale i PHP zaczyna pracę; kiedy kończy, bajty odpowiedzi idą prosto z powrotem.
...
Zanim pójdziesz dalej, poznaj kilka ograniczeń — nie da się ich obejść:
...
To dwa pierwsze szczeble dłuższej drabiny — `Classic → SAPI Worker → PSR Worker → Async` — na której każdy kolejny stopień oddaje PHP większą kontrolę nad cyklem życia żądania. Na razie gotowe są tylko dwa pierwsze; [Tryby wykonania](/pl/docs/execution-modes) opisują całą drabinę i podpowiadają, jak rozpoznać, którego szczebla może użyć twoja aplikacja.

::: tip
Szczebel, którego może użyć aplikacja, zależy od niej samej, a nie od ograniczeń narzuconych przez Rapirę. Globalny stan, który nie przeżywa drugiego żądania, trzyma cię na trybie Classic — to ograniczenie twojego kodu i da się je naprawić.
:::
