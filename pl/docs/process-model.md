...
Po uruchomieniu Rapiry masz jeden proces nadrzędny i pulę workerów. Proces nadrzędny trzyma wszystko, co może istnieć tylko w jednym egzemplarzu — nasłuchujące gniazdo, obraz silnika PHP, pidfile — a potem forkuje; workery zajmują się żądaniami. Żadne żądanie nie wędruje z procesu do procesu: workery *są* kopiami procesu nadrzędnego, sforkowanymi już po podniesieniu PHP, i każdy z nich zdejmuje swoje połączenia prosto z gniazda.

Ten układ wygląda tak samo niezależnie od tego, czy pracujesz na szczeblu [Classic](/pl/docs/classic), czy [SAPI Worker](/pl/docs/worker). [Tryb wykonania](/pl/docs/execution-modes) decyduje o tym, co dzieje się wewnątrz workera przy każdym żądaniu; nie zmienia natomiast tego, jak pula powstaje, jak jest nadzorowana i jak się ją przeładowuje.
...
Każdy worker to jeden interpreter PHP w wersji NTS z własnym asynchronicznym runtime'em HTTP, przyjmujący połączenia na odziedziczonym gnieździe. Z przodu nie stoi żaden dyspozytor, który rozdzielałby ruch: wszystkie workery czekają w `accept()` na tym samym gnieździe, a jądro systemu oddaje każde przychodzące połączenie dokładnie jednemu z nich.

Proces nadrzędny nigdy nie obsługuje żądania. Nie ma nawet stosu HTTP — to jeden wątek siedzący w `poll(2)` na self-pipe i czekający na sygnały, śmierć potomków i własne timery. Tak ma być: proces, który musi przeżyć, żeby zrestartować całą resztę, robi możliwie najmniej.
...
## Czego pilnuje proces nadrzędny
...
`pool.mode` decyduje o tym, jak pula dobiera swój rozmiar. W każdym trybie liczy się `pool.processes` — dla `static` to dokładna liczba procesów, dla pozostałych dwóch sufit.
...
**`static`** to ten nudny i przewidywalny — i właśnie dlatego jest dobrym domyślnym wyborem dla większości wdrożeń: zużycie pamięci jest płaskie, a worker, który padnie, po prostu dostaje następcę.
...
Sygnałami sterujesz działającym serwerem: zatrzymujesz go, przeładowujesz, pytasz, co właśnie robi. Wszystkie trafiają do **procesu nadrzędnego**.
...
::: warning
Sygnały wysyłaj do procesu nadrzędnego, nigdy do pojedynczego workera. Workery `SIGUSR1` i `SIGUSR2` po prostu ignorują, a `SIGTERM` traktują jak natychmiastowe ubicie — właśnie tym posługuje się nadzór nad żądaniem, gdy żądanie ma zginąć *już teraz*. Ręczne sygnalizowanie workera omija wszystkie gwarancje opisane na tej stronie.
:::
...
Domyślnie jest to jeden na rdzeń CPU i dla `static` to dobry punkt wyjścia. PHP działa synchronicznie, więc worker obsługuje jedno żądanie naraz: tam, gdzie żądania spędzają większość czasu na czekaniu na bazę albo zewnętrzne API, workerów zwykle przyda się więcej niż rdzeni; tam, gdzie całą robotę wykonuje procesor — rzadko kiedy. Liczbę ustawisz kluczem `pool.processes` — jako dokładną wartość w `static` albo jako sufit w `dynamic` i `ondemand`.
:::
