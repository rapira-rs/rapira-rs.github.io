...
Большинству эта страница никогда не понадобится: возьмите готовый бинарник со страницы [Установка](/ru/docs/installation) — и на этом всё. Собирать Rapira самому имеет смысл в тех случаях, которые готовые артефакты не покрывают, и ничего сложного в этом нет: единственное новое требование — PHP, который Rapira сможет встроить. Собирается Rapira на Linux и macOS.
...
Rapira не обращается к PHP через сокет, а линкует интерпретатор прямо в свой процесс. Значит, PHP должен существовать в виде разделяемой библиотеки: **версия 8.4 или 8.5, сборка NTS (непотокобезопасная), собранная с `--enable-embed=shared`** — именно этот флаг и даёт `libphp.so` (на macOS — `libphp.dylib`).
