...
Rapira собирается из исходников на Linux и macOS. Самостоятельная сборка закрывает те случаи, которые не покрывают готовые бинарники со страницы [Установка](/ru/docs/installation), а единственное требование сверх обычных инструментов Rust и C — это PHP, который Rapira сможет встроить.
...
В результате получается тот же сервер, что ставится из пакетов: [Быстрый старт](/ru/docs/quickstart) проведёт через первый скрипт, [Командная строка](/ru/docs/cli) перечисляет всё, что принимает `serve`, а [Конфигурация](/ru/docs/configuration) разбирает `rapira.toml`.
