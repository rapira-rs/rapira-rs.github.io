...
Файл конфигурации содержит все настройки сервера. Значение в файле переопределяет встроенное значение по умолчанию. `RUST_LOG` и `NO_COLOR` меняют только вывод журнала. Все ключи и форматы адреса `listen` перечислены в разделе [Конфигурация](/ru/docs/configuration).
