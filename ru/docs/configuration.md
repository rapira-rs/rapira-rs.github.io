...
В файле четыре секции, и каждая из них необязательна: `[http]` настраивает слушающий сокет, `[pool]` - рабочие процессы, `[supervisor]` - мастер-процесс, `[log]` - то, что уходит в stderr. У входного PHP-скрипта нет значения по умолчанию. Задайте `pool.entrypoint` или передайте скрипт позиционным аргументом в командной строке.
