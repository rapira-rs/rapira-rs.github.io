...
| `[log.targets]` | таблица «цель → уровень» | пусто | Точечные переопределения поверх `level`. Каждый ключ называет одну из целей, под которыми пишет Rapira: `php` несёт собственный вывод PHP, `http` и `grpc` - вывод серверов соответствующих протоколов, а `net` - записи цикла приёма соединений. Ключ сопоставляется по префиксу, поэтому `php` захватывает заодно и `php_sys::callbacks`, и всё, что ниже. Полный список целей есть в разделе [Логирование](/ru/docs/logging). |
