...
`[http]` и `[grpc]` настраивают отдельные слушатели и пулы PHP-воркеров. Настройте хотя бы одну из этих секций. `[supervisor]` настраивает мастер-процесс. `[log]` настраивает вывод в stderr.

Каждому включённому слушателю нужен входной PHP-скрипт. Задайте `http.pool.entrypoint`, `grpc.pool.entrypoint` или оба ключа.
...
Эта секция включает нативный унарный gRPC по незашифрованному HTTP/2. Полный пример PHP-сервиса и клиентские команды приведены в разделе [gRPC](./grpc).

| Ключ | Тип | По умолчанию | Что делает |
| --- | --- | --- | --- |
| `listen` | строка | `"127.0.0.1:9001"` | Адрес TCP или путь к сокету `unix:`. Использует тот же синтаксис адреса, что и `http.listen`. |
| `protos` | список строк | нет - ключ обязателен | Каталоги для рекурсивного поиска файлов `.proto` приложения. Список должен содержать хотя бы один каталог. |
| `import_paths` | список строк | пусто | Дополнительные каталоги для импортируемых схем. Сервисы из файлов, доступных только через импорт, не становятся маршрутами приложения. |
| `reflection` | логическое | `true` | Включает сервисы рефлексии v1 и v1alpha. |
| `interceptors` | список строк | пусто | Самостоятельный бинарный файл принимает пустой список. Хосты на Rust могут предоставлять перехватчики через API плагина. |
| `max_request_message_size_mb` | целое | `4` | Ограничение размера сообщения запроса в МиБ. Не меньше 1. И сжатые, и несжатые данные должны укладываться в лимит. |
| `max_response_message_size_mb` | целое | `4` | Ограничение размера сообщения ответа в МиБ. Не меньше 1. И сжатые, и несжатые данные должны укладываться в лимит. |

Пути вычисляются относительно каталога файла конфигурации. Корневые каталоги импорта используются в порядке конфигурации: сначала `protos`, затем `import_paths`. Стандартные импорты `google/protobuf` встроены. Некорректные схемы, отсутствующие импорты, конфликтующие определения и потоковые методы приложения останавливают инициализацию.

### Таблица `[grpc.compression.gzip]`

| Ключ | Тип | По умолчанию | Что делает |
| --- | --- | --- | --- |
| `enabled` | логическое | `false` | Включает сжатие gzip для ответов приложения, если клиент указывает поддержку gzip в `grpc-accept-encoding`. Запросы со сжатием gzip принимаются при любом значении. |
...
HTTP и gRPC могут работать вместе. Каждый слушатель использует собственный пул и входной скрипт. Перезапустите Rapira для загрузки изменённых схем.
...
Пути файловой системы включают входные скрипты обоих пулов, `grpc.protos`, `grpc.import_paths`, `supervisor.pidfile`, `http.static.root`, `http.sendfile.root` и `http.uploads.dir`. Каждый относительный путь использует каталог файла конфигурации как базовый. Относительные пути слушателей `unix:` также используют этот каталог. Например, значение `entrypoint = "app/worker.php"` в `/etc/rapira/rapira.toml` даёт путь `/etc/rapira/app/worker.php`.
