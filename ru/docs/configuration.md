...
Чтобы запустить Rapira, файл конфигурации не нужен: у `rapira serve app/worker.php` на всё найдётся значение по умолчанию. `rapira.toml` появляется тогда, когда этих значений перестаёт хватать: нужен другой адрес, фиксированное число воркеров, политика пересоздания процессов, pidfile, который прочитает система инициализации, уровень логирования, который наконец что-то расскажет. Покажите серверу файл — и дальше он читает настройки оттуда:

```bash
rapira serve --config /etc/rapira/rapira.toml
```

В файле четыре секции, и каждая из них необязательна: `[http]` настраивает слушающий сокет, `[pool]` — рабочие процессы, `[supervisor]` — мастер-процесс, `[log]` — то, что уходит в stderr. Единственное, что Rapira не может придумать за вас, — входной PHP-скрипт: задайте его ключом `pool.entrypoint` или передайте позиционным аргументом в командной строке.

::: info
Настройки складываются слоями: флаг командной строки сильнее файла конфигурации, а файл сильнее встроенного значения по умолчанию. Поэтому `--processes 8` перебивает `processes = 4` из файла, и конфигурацию, которая лежит в системе контроля версий, всё равно можно подкрутить на один запуск. Сами флаги описаны в разделе [Командная строка](/ru/docs/cli).
:::

## Полный rapira.toml

Все ключи, которые понимает Rapira, собраны в одном файле. Обязательного здесь нет ничего: удалите любую строку, и вступит в силу значение по умолчанию. Исключений два — у `pool.entrypoint` значения по умолчанию попросту нет, а `min_spare` и `max_spare` обязательны, пока в файле стоит `mode = "dynamic"`.
...
Дальше на этой странице — тот же самый файл, ключ за ключом.

## Секция `[http]`

Парадный вход: где Rapira слушает, что окружение запроса рассказывает PHP о сервере, под которым он работает, и сколько тела запроса сервер готов прочитать.
...
`server_name` и `server_port` влияют только на то, что PHP увидит в `$_SERVER`, и никак не меняют адрес, который сервер занимает на самом деле. За это отвечает `listen` — и только он.

## Секция `[pool]`

Воркеры — это процессы, которые непосредственно выполняют PHP, а секция говорит, что именно они выполняют, сколько их и когда мастер убирает одного из них. Как мастер распоряжается этими числами, объясняет [Модель процессов](/ru/docs/process-model); здесь это просто ключи.
...
| `classic` | логическое | `false` | `false` оставляет воркер живым между запросами (ступень SAPI Worker); `true` выполняет входной скрипт с нуля на каждый запрос, ровно как это делал бы php-fpm. Смотрите [Режимы выполнения](/ru/docs/execution-modes). Флаг `--classic` умеет только включать режим: `true`, записанный здесь, из командной строки уже не отменить. |
...
| `max_spare` | целое | нет | Только для `dynamic`, и там обязателен: сокращать число простаивающих воркеров до этого значения. Пара должна укладываться в `1 <= min_spare <= max_spare <= processes`, а в любом другом режиме каждый из этих ключей — ошибка, а не безобидная лишняя строка. |
...
Правила для мастер-процесса — того самого, который держит слушающий сокет, присматривает за воркерами и принимает ваши сигналы. С ним же общается система инициализации, поэтому именно эту секцию обычно и заполняют, когда пишут unit-файл; смотрите [Развёртывание](/ru/docs/deployment).
...
Rapira пишет всё в stderr, по одной операции записи на сообщение, поэтому вывод мастера и воркеров не перемешивается посреди строки. Эта секция решает, насколько громким получится поток и как выглядит каждая запись; про отдельные цели, форматы и про то, как диагностика PHP раскладывается по уровням, рассказывает [Логирование](/ru/docs/logging).
...
Со значениями всё устроено так же. `level = "verbose"`, `format = "pretty"` и `unsafe_field_names = "allow"` — жёсткие ошибки, а не тихий откат к значению по умолчанию: защитный фильтр, переживший опечатку, куда хуже того, который отказывается стартовать у вас на глазах. У чисел тоже есть границы: `pool.processes` и `http.max_body_size_mb` не меньше 1, а любой ключ `*_secs` ограничен сверху значением `86400` — это сутки.

::: warning
Проверка идёт до того, как что-либо запустится, поэтому нераспознанный ключ останавливает старт, а не тихо ухудшает работу сервера. Об этом стоит помнить, когда вы правите `rapira.toml` на машине, которая прямо сейчас обслуживает трафик: работающий процесс останется нетронутым, но следующий запуск обязан пройти успешно.
:::
...
Позиционный аргумент `SCRIPT` устроен наоборот. Это значение из командной строки, поэтому относительный путь в нём считается от текущего каталога — ровно так же, как у любой другой программы, которой вы называете имя файла.
...
::: question А файл конфигурации вообще нужен?
Нет. `rapira serve` со скриптом и парой флагов закрывает обычный случай, а всё, что вы не задали, берётся из описанных выше значений по умолчанию. Файл начинает окупаться, когда настроек становится больше, чем хочется держать в голове, или когда их нужно ревьюить и хранить в системе контроля версий вместе с приложением.
:::

::: question Можно ли настроить Rapira через переменные окружения?
Нет: настройки берутся из файла конфигурации и флагов командной строки, и больше ниоткуда. Исключение — две переменные, которые касаются только логов. `RUST_LOG` целиком заменяет фильтр логов, так что ради шумной отладочной сессии не придётся править конфигурацию. `NO_COLOR` убирает цвет из формата `plain`: любое непустое значение выключает его даже в терминале. Обе описаны в разделе [Логирование](/ru/docs/logging).
:::

::: question Почему сервер не стартует с `mode = "dynamic"`?
Скорее всего, дело в числе свободных воркеров. Режиму `dynamic` нужны оба ключа, `min_spare` и `max_spare`, и они должны укладываться в `1 <= min_spare <= max_spare <= processes` — учтите, что флаг `--processes` опускает тот самый потолок, с которым их сверяют. В режимах `static` и `ondemand` те же ключи отвергаются наотрез, и обычно это значит, что в строке `mode` написано не то, что задумывалось.
:::
