...
Эта страница описывает функции для авторов документации. Каждый пример создан из показанной разметки Markdown.

Выполните `npm ci`, чтобы установить зафиксированные зависимости. Затем выполните `npm run dev`. Откройте локальный адрес из вывода команды. Каталоги переводов имеют ту же структуру, что и исходные английские файлы.
...
После типа можно задать конкретный заголовок:
...
Встроенные бейджи могут показывать метки статуса:
<Badge type="tip" text="новое" /> <Badge type="warning" text="бета" /> <Badge type="danger" text="устарело" />
