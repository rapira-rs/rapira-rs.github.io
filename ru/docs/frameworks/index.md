...
Middleware отвечает на запрос только тогда, когда путь ведёт к файлу внутри этого корня. Список `forbid` по умолчанию не выпускает наружу файлы `.php`, поэтому фронт-контроллер из `public/` файлом не отдаётся никогда. Любой другой URL выполняет входной скрипт — одинаково в режимах Classic и Worker, — а куда именно хотел попасть клиент, приложение узнаёт из `$_SERVER['REQUEST_URI']`. URL каталога тоже выполняет входной скрипт: индексный файл middleware для него не отдаёт.
