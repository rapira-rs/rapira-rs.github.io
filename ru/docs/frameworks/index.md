...
**В режиме воркера процесс остаётся живым.** Ваш скрипт один раз поднимает приложение и уходит в цикл, на каждой итерации запрашивая у Rapira следующий запрос. Фреймворк между запросами больше не разбирается. Где этот режим стоит среди трёх, показывают [Режимы выполнения](/ru/docs/execution-modes), а справочник по его API — [Режим воркера](/ru/docs/worker).

Одна и та же кодовая база работает в обоих режимах: оставьте `public/index.php` как есть и положите рядом `worker.php`. У проверенных приложений на Symfony и Yii3 оба файла лежат рядом, а какой из них запустится, выбирает флаг `--mode`: `rapira serve --mode classic public/index.php` или `rapira serve --mode worker worker.php`. На время переезда классический режим остаётся под рукой как откат.
...
Middleware отвечает на запрос только тогда, когда путь ведёт к файлу внутри этого корня. Список `forbid` по умолчанию не выпускает наружу файлы `.php`, поэтому фронт-контроллер из `public/` файлом не отдаётся никогда. Любой другой URL выполняет входной скрипт — одинаково в классическом режиме и в режиме воркера, — а куда именно хотел попасть клиент, приложение узнаёт из `$_SERVER['REQUEST_URI']`. URL каталога тоже выполняет входной скрипт: индексный файл middleware для него не отдаёт.
