---
title: Symfony
description: "Как запустить приложение Symfony на Rapira в режиме воркера: скрипт воркера, сброс сервисов между запросами и то, как значения из .env попадают в контейнер."
---
...
Ядро поднимается в начале скрипта, вне цикла, и живёт столько же, сколько процесс воркера: автозагрузчик, скомпилированный контейнер, маршрутизатор, диспетчер событий и все соединения, открытые вашими бандлами, строятся один раз, а не на каждый запрос. Это и даёт [режим воркера](/ru/docs/worker); подробнее — в разделе [Режимы выполнения](/ru/docs/execution-modes).
...
`--mode worker` включает режим воркера, а `127.0.0.1:8000` — адрес прослушивания по умолчанию. `rapira serve` остаётся на переднем плане, а `Ctrl-C` мягко его останавливает, дав доработать начатым запросам.
