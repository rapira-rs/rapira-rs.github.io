...
Скрипт воркера — дополнительная точка входа, а не замена фронт-контроллеру, поэтому `public/index.php` стоит оставить: именно его запускает режим Classic, и он по-прежнему удобен для локальной работы со встроенным сервером PHP.
