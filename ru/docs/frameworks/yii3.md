...
На стороне PHP ставить нечего: скрипт воркера ниже — единственный новый файл в проекте, и лежит он в корне проекта, рядом с `composer.json`, потому что `rootPath` раннера — это как раз корень проекта.
...
Контейнер каждый раз собирается заново, поэтому подвижных частей меньше, сброс негде сделать неправильно и состоянию неоткуда протечь из одного запроса в следующий. Этот вариант тоже прошёл весь набор проверок.
