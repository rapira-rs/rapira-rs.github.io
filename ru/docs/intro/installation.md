...
Каждый релиз использует последнюю доступную патч-версию ветки PHP. В архиве точная версия записана в `share/php/PHP_VERSION.txt`.
На работающем сервере версию сообщают `PHP_VERSION` и `phpinfo()`.
