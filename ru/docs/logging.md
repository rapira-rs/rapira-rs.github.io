...
Если аргумент опущен, запись пишется на уровне `Info`. Поскольку уровни те же самые, что и везде, `[log.targets]` и `RUST_LOG` фильтруют записи приложения ровно так же, как записи самого сервера: `app = "debug"` в `[log.targets]` поднимает записи приложения, не трогая ничего вокруг.
