...
Завершение запросов и нативных операций вызывает создание накопительных снимков метрик. Ресурсы OTLP содержат `service.name`, `process.pid` и `rapira.role`. Каждый процесс также имеет постоянный случайный `service.instance.id`. Ресурсы воркера также содержат `rapira.pool`.
