...
`pool.mode` выбирает, как пул определяет свой размер. Главное число в любом режиме — `pool.processes`: для `static` это точное количество, для остальных двух — потолок; по умолчанию это один воркер на логическое ядро.
...
В режиме Classic входной скрипт выполняется с нуля на каждый запрос, поэтому заменять нечего: новый код начинает работать без перезагрузки. Исключение — `opcache.validate_timestamps = 0`: с ним сегмент OPcache, созданный в мастере, продолжает отдавать старые опкоды до полного перезапуска. В режиме SAPI Worker приложение поднимается один раз и остаётся в памяти, поэтому выложенный код начинает работать только после плавной перезагрузки — сделайте её шагом деплоя. Подробнее об этом — в разделе [Запуск в продакшене](/ru/docs/deployment).
