...
两种情况都请在安装前用 `rapira-v0.6.0-SHA256SUMS.txt` 核对一遍，命令见[验证校验和](#验证校验和)。
...
构建只发布在 GitHub Releases，别处没有。目前还没有 apt 或 yum 仓库，所以升级就是下载新产物、覆盖旧的装上去，而不是执行 `apt upgrade`。软件包会就地替换已安装的版本；用压缩包的话，把新目录解压到旧目录旁边，再把符号链接切过去——原来的目录树还在，回滚只要一条命令。
