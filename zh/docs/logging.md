...
Rapira 把所有内容都写进同一条流：服务器自身的生命周期事件、master 的监管决策、HTTP 接入层，还有 PHP 的诊断信息——统统写到 stderr，统统由同一套过滤规则决定去留。PHP 也不例外：PHP 报一条警告，不用你翻到另一个 `error_log` 文件里去找，它和别的记录一样躺在同一份日志里，级别也和别的记录一样可以调高或调低。

默认是刻意安静的。开箱只有 `error` 能过，因为一台在生产机器上不停写日志的服务器，日志根本没人看。想提高级别，配置里加一行就够；一点都不想碰配置文件的话，还有个环境变量可用。
...
一个全局级别往往不够用。追查 PHP 里的问题时，你想把 PHP 的诊断信息开到 `debug`，又不想把 HTTP 那一层的内部细节一并调高。`[log.targets]` 就是干这个的：
...
依赖库以自己的模块路径作为目标——`pingora_core`、`tokio` 等等——过滤方式完全一样。哪个库在日志里写得太多，它的目标名就明明白白写在那条记录上，直接填进 `[log.targets]` 即可。
...
弃用提醒落在 `debug`，是为了让 vendor 里躺着几千条弃用提醒的代码库，不至于把你真正需要看见的那两条警告埋掉。
...
基本可以肯定没有：`level` 默认就是 `error`，所以一台健康的服务器本来就不会写出什么。用 `RUST_LOG=info` 启动，启动过程、监听器和 worker 生命周期就都出来了。
:::
