...
Rapira 把所有内容都写进同一条流：服务器自身的生命周期事件、master 的监管决策、HTTP 接入层，还有 PHP 的诊断信息——统统写到 stderr，统统由同一套过滤规则决定去留。PHP 的一条警告就是这份日志里的一条记录，而不是另一个 `error_log` 文件里的一行，它的级别和别的记录一样可以调高或调低。
