---
title: 快速开始
description: 用 Rapira 跑起你的第一个 PHP 应用：经典模式下的前端控制器、同一个应用的常驻 worker 版本，以及一份五行的 rapira.toml。
---

# 快速开始

本页接着[安装](/zh/docs/installation)往下讲：此时你手上已经有一个能用的 `rapira` 可执行文件，以及它自带的 PHP。接下来的几分钟里，你会先用经典模式返回一个页面，再把同一个应用改造成常驻 worker，最后把命令行上的设置搬进配置文件。
...
SAPI Worker 这一级会让脚本一直活着：它只启动一次，随后在循环里不断向 Rapira 要下一个请求；Rapira 重新填好超全局变量，再调用你的处理函数。PHP 代码的模样还是熟悉的那套——照样读 `$_GET`，照样 `echo` 出响应——区别在于启动开销从每个请求付一次变成每个进程只付一次。整道阶梯见[执行模式](/zh/docs/execution-modes)。
...
`create_plugin_handler()` 向服务器要一个负责处理 HTTP 的 handler，具体要哪一个由 `HttpHandlerConfig` 指明。之后 `handleRequest()` 会一直阻塞到请求到来，用你的回调处理它，再返回 `true`；服务器开始关闭时它返回 `false`，循环也就到此为止。
...
这五行只是配置文件所能接受的一小部分：进程池的伸缩模式、worker 回收、请求超时、日志、supervisor 的 pidfile。不认识的键会被直接拒绝而不是忽略，所以拼错一个字母会让启动失败，而不是悄无声息地不起作用。完整的参考见[配置](/zh/docs/configuration)，命令行参数见[命令行](/zh/docs/cli)。
...
- [框架集成](/zh/docs/frameworks/)——Symfony、Laravel 和 Yii3 的现成入口脚本。

::: question 运行 worker 脚本需要 Composer 吗？
不需要。`create_plugin_handler()`、`HttpHandlerConfig` 和各个 handler 类都来自 Rapira 启动解释器时注册的那个 PHP 模块，所以上面这段脚本一个自动加载器都不用就能跑。真实的应用当然还是要 `require` 自己的 `vendor/autoload.php`——放在循环之前，这样只付一次开销。
:::
