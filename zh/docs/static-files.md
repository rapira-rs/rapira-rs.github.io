...
`forbid` 包含中间件不提供的扩展名。默认值为 `[".php"]`。
显式列表会替换默认值。例如，`forbid = [".php", ".env"]` 会阻止两个扩展名。
...
中间件将每个条目与文件名末尾比较。分隔符或空格不能位于文件名末尾。
因此，此类条目无法保护文件。验证会拒绝它。
:::
...
它会在拒绝新文件前删除过期条目。每个 worker 最多使用 16 MiB 缓存。
重启会清空缓存。

每个 worker 验证自己的条目。删除的文件最多在一秒后影响响应。
更改的文件仅在修改时间或长度变化时影响响应。
如果修改时间和长度不变，权限更改不会删除条目。
删除文件或重启服务器可删除条目。替换文件时，请更改修改时间或长度。
